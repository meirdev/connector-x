...
/// Convert ClickHouse data types to Arrow data types.
pub struct ClickHouseArrowTransport;

impl_transport!(
    name = ClickHouseArrowTransport,
...
impl TypeConversion<Uuid, String> for ClickHouseArrowTransport {
    fn convert(val: Uuid) -> String {
        val.to_string()
    }
}

impl TypeConversion<IpAddr, String> for ClickHouseArrowTransport {
    fn convert(val: IpAddr) -> String {
        val.to_string()
    }
}

impl TypeConversion<Vec<Option<i8>>, Vec<Option<i16>>> for ClickHouseArrowTransport {
    fn convert(val: Vec<Option<i8>>) -> Vec<Option<i16>> {
        val.into_iter().map(|opt| opt.map(|v| v as i16)).collect()
    }
}

impl TypeConversion<Vec<Option<u8>>, Vec<Option<u16>>> for ClickHouseArrowTransport {
